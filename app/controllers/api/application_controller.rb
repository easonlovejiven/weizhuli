...
#  before_filter :set_current_user,  :set_user
  ...
  def set_weibo_uid
    case "#{params[:u].strip} #{params[:p].strip}"
    when "intelbiz 778899" # name & password
      @uid = "2295615873" #英特尔商用频道
    when "intelsocial 778899" # name & password
      @uid = "intelsocial" #英特尔商用频道
    else
      render :text=>"Authentification failed"
    end
  end
