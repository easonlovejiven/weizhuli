  ...
  def create
    ...
    respond_to do |format|
      if @news_category.save
        format.html { redirect_to([:admin,@news_category], :notice => 'News category was successfully created.') }
        format.xml  { render :xml => @news_category, :status => :created, :location => [:admin,@news_category] }
        format.js {
          render :update do |page|
            page.call "after_create_news_category",@news_category.to_json
          end
        }
      else
        ...
        format.js {
          s = render_to_string  :action=>"new", :layout=>false
          render :update do |page|
            page.replace_html "fancybox-content", s
          end
        }
      end
    end
