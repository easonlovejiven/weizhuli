# -*- encoding : utf-8 -*-
class WeiboHighLevelFans < ActiveRecord::Base
  attr_accessible :by_uid, :follow_time, :uid, :level, :total_interactions, :year_interactions, :month_interactions, :week_interactions
  ...
  scope :join_remarks, ->{
    joins("left join weibo_user_remarks on weibo_user_remarks.uid = weibo_high_level_fans.by_uid and weibo_user_remarks.user_id = #{current_user_id}")
  }
  scope :has_email, ->(is){
    if is_true(is)
  ...
  scope :level, ->(l){
    where('weibo_high_level_fans.level = ?', l)
  }
