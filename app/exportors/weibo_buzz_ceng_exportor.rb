...
EOF
  ...
  def get_buzz_cengji(urls)
    ...
    Rseg.load
    #全局变量统计关键词数量
      keywords = ['安腾','Itanium','凌动,','Atom','Xeon','酷睿i3','酷睿i5','酷睿i7','Core i3','Core i5','Core i7','Intel','英特尔','Intel 平板','英特尔 平板','PC平板2合1','PC平板二合一','二合一超极本','二合一电脑','平板PC2合1','平板PC二合一']#文件的路径
      words_stats = {}
      keywords.each{|word|
        word = word.strip
        words_stats[word] = 0
      }
      rows << %w{URL 传播层级 BUZZ声量}
      urls.each do |url|
