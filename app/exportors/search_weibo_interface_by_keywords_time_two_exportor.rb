# -*- encoding : utf-8 -*-
class SearchWeiboInterfaceByKeywordsTimeTwoExportor < SearchBase
...
EOF
  ...
  before do |this,opts|
    ...
    #这快儿要做个处理
    @no_sources = opts[:no_sources] #跳过这些来源
    #@no_sources = @no_sources.join(",")
    
  end

  export name:"提取关键词微博" do |ins,opts|
    ins.export_search_weibo_interface(@keywords,opts)
  end

# 导出 uids 的粉丝列表
  def export_search_weibo_interface(keywords,opts)
    te = TextEvaluate.new
      start_time = opts[:start_time]
      end_time = opts[:end_time]
      sum = opts[:sum]
            ...
            keywords.each do|keyword|
              ...
              number = 0
                ...
                while (processing == true) do
                  pas={access_token:"2.002F_9jFDtpNADac000c4d2dEEO6kB",q:q,endtime:endtime,count:50,antispam:0,dup:0} #定义一个hash 用来存参数用 to_query 方法追加
                  ...
                  if !res.statuses.blank?
                    ...
                    res.statuses.each do|status|
                      ...
                      puts "number:#{number}"
                      uids << user.id
                      rows << [user.id,user.screen_name,user.location,gender,user.followers_count,user.friends_count,user.statuses_count,Time.parse(user.created_at),verified_type,user.verified_reason] + [keyword, url1,is_forward,created_at,status.text,source,status.reposts_count,status.comments_count, eva, eva_word]
