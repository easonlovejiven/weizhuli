  ...
  filename = "用户活跃度.csv"
  CSV.open filename,"wb" do |csv|
  @uids = open '/home/rails/Desktop/uid'#文件的路径
  @uids = @uids.read
  @uids = @uids.strip.split("\n")
   csv << %w{uid 活跃度 关注时间}
    @uids.each do |uid|
      ...
      if w_a.forward_average && w_a.comment_average == nil
        w_a.update_average
      if wurs[0].blank?
        csv << [uid,w_a.forward_average+w_a.comment_average,nil]
      elsif
        @follow_time = wurs[0].follow_time.strftime("%Y-%m-%d %H:%M:%S")
        csv << [uid,w_a.forward_average+w_a.comment_average,@follow_time]
     end   
    end
   end
  end
...
Qualcomm中国 1738056157  60221/261895   0.229(今年的) 未认证的:201674
  ...
  CSV.open filename,"wb" do |csv|
    ...
    #去除数组当中的重复元素
   filename = "intel-uniq-uid.csv"
   CSV.open filename,"wb" do |csv|
    csv << %w{元素}
    @uids = open '/home/rails/Desktop/intel-uniq-uid'#文件的路径
    @uids = @uids.read
    @uids = @uids.strip.split("\n")
    @uids = @uids.uniq
    @uids.each do |uid|
  ...
  filename = "2014第二季度.csv"
  CSV.open filename,"wb" do |csv|
    csv << %w{关键词名称 数量统计}
    task = GetUserTagsTask.new
    @keywords = open '/home/rails/Desktop/keywords'#文件的路径
    @keywords = @keywords.read
    @keywords = @keywords.strip.split("\n")
    @keywords.each do |keyword|
      url = "https://c.api.weibo.com/2/search/statuses/limited.json?"
      start_time = Time.parse("2014-04-01 00:00:00").to_i
      end_time = Time.parse("2014-07-01 00:00:00").to_i
      pas={access_token:"2.00IQlCdDDtpNAD9d1d70bf2806YTGp",q:keyword,starttime:start_time,endtime:end_time,count:50}
...
avg_forward_rate,avg_comment_rate,avg_forward_average,avg_comment_average,avg_interactive,evaluates,origin_rate,day_posts,day_posts_in_weeks]      
  ...
  end_time = '2014-10-20'
  filename = "intel互动内容1172.csv"
  CSV.open filename,"wb" do |csv|
    csv << %w{UID URL 内容 动作}
    uids = open '/home/rails/Desktop/hudong-uid'
    uids = uids.read
    uids = uids.strip.split("\n")
    uids = uids.uniq
    uids.each do |uid|
      uid = uid.strip
      comments = WeiboComment.where("uid = ? and comment_uid = ? and comment_at between ? and ?",2637370927,uid,start_time,end_time)
        if !comments.blank?
          comments.each do |comment|
      ...
      forwards = WeiboForward.where("uid = ? and forward_uid = ? and forward_at between ? and ?",2637370927,uid,start_time,end_time)
      if !forwards.blank?
        forwards.each do |forward|
          fc = MForward.find(forward.forward_id)
  ...
  CSV.open filename,"wb" do |csv|
    csv << %w{UID}
      quan2 = open '/home/rails/Desktop/quan2'
      quan2 = quan2.read
      quan2 = quan2.strip.split("\n")

      quan = open '/home/rails/Desktop/quan'
      quan = quan.read
  ...
  filename = "intel的这些核心粉丝二次转发(10).csv"
  CSV.open filename,"wb" do |csv|
    uid_url = '/home/rails/Desktop/uids'
    start_time = '2014-10-01'
    end_time = '2014-10-14'
    target = 2637370927  #英特尔商用频道
    csv = %w{uid 二次转发}
    @uids = open uid_url
    @uids = @uids.read
    @uids = @uids.strip.split("\n")
  ...
  filename = "intel-有效性-情感分析10月.csv"
  CSV.open filename,"wb" do |csv|
  ...
  page = 1
  while true
    comments = WeiboComment.where("uid = ? and comment_uid = ? comment_at between ? and ?",2637370927,uid,start_time,end_time).paginate(per_page:1000, page:page)
    break if comments.blank?
    comments.each do |comment|
      comment_date = comment.comment_at  
      url = "http://weibo.com/"+uid.to_s+"/"+WeiboMidUtil.mid_to_str(comment.weibo_id.to_s)
    ...
    page = 1
   while true
    forwards = WeiboForward.where("uid = ? and forward_uid = ? and forward_at between ? and ?",2637370927,uid,start_time,end_time).paginate(per_page:1000, page:page)
    break if forwards.blank?
    forwards.each do |forward|
      forward_date = forward.forward_at
      url = "http://weibo.com/"+uid.to_s+"/"+WeiboMidUtil.mid_to_str(forward.weibo_id.to_s)
  ...
  target_uids.each do |id|
    fans = WeiboUserRelation.where("uid = ? and follow_time between ? and ?",id,'2014-09-01','2014-09-30')
    fans.each do |fan|
  ...
  CSV.open filename,"wb" do |csv|
    csv << %w{URL}
    old_csv = CSV.open '/home/rails/server/weibo-marketing/111.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
    lines = old_csv.read
    lines.each do |line|
      content = line[0]
      num = content.index("http").to_i
      url = content[num,content.length]
      csv << [url]
    end
  end
...
#统计每一个人与中国的互动量，转发＋评论 大于 1的人，再更新他的活跃度，并计算7天发贴量大于1 的人，把用户信息导出到另一个CSV里
  ...
  filename = '十月份(全部).csv'
  CSV.open filename,"wb" do |csv|
  one_csv = CSV.open '/home/rails/server/weibo-marketing/73066与中国的有互动的粉丝信息最新11-十月份.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
  lines1 = one_csv.read
    lines1.each do |line|
      csv << line
    end 
  two_csv = CSV.open '/home/rails/server/weibo-marketing/73066与中国的有互动的粉丝信息最新22-十月份.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
  lines2 = two_csv.read
    lines2.each do |line|
      csv << line
    end
  three_csv = CSV.open '/home/rails/server/weibo-marketing/73066与中国的有互动的粉丝信息最新33-十月份.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
  lines3 = three_csv.read
    lines3.each do |line|
      csv << line
    end
  end
  ...
  status = {}
  num = 0
  uids = open '/home/rails/Desktop/uids-xph'#文件的路径
  uids = uids.read
  uids = uids.strip.split("\n")
    uids.each do |uid|
      num += 1
  ...
  CSV.open filename,"wb" do |csv|
    csv << %w{uid 排列}
    num = 0 
    old_csv = CSV.open '/home/rails/Desktop/2637370927_fans.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
    lines = old_csv.read
    lines.each do |line|
      num += 1
      csv << [line[0],num]
  ...
  CSV.open filename,"wb" do |csv|
    ...
    numa = 0
    numb = 0
    old_csv = CSV.open '/home/rails/Desktop/xph-time.csv'#(这个csv文件是没有对内容正负面影响进行统计的csv文件)
    lines = old_csv.read
    lines.each do |line|
      one = line[2].to_i
      if one <= 30
  ...
  #
  filename = "xph-time.csv"
  CSV.open filename,"wb" do |csv|
  csv << %w{UID 最近一条微博时间 距离现在天数}
  day = 0
  num = 0
  task = GetUserTagsTask.new #最近一条微博时间要从接口当中提取
  old_csv = CSV.open '/home/rails/Desktop/xph.csv'
  lines = old_csv.read
    lines.each do |line|
  begin
      num = line[1].to_i%10
      if num == 0
        ...
        if !res.blank?
           if !res.status.blank?
              last_time = DateTime.parse(res.status.created_at).strftime("%Y-%m-%d %H:%S")
              now_time = Time.now.strftime("%Y-%m-%d %H:%S")
              lt = Date.parse(last_time)
              nt = Date.parse(now_time)
              day = (nt-lt).to_i
        ...
        if e.message =~ /User does not exists!/
          csv << [uid,"此用户不存在"] #被屏蔽的用户还可以在库里查看到用户信息,但从接口当中就取不到这个用户的信息
        end
      end
    end
  end
