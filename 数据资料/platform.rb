...
#===================================>>
  ...
  #不通过接口或者平台
    filename = "互动用户基本信息BBB2.csv"
    CSV.open filename,"wb" do |csv|
      csv << %w{UID 昵称 位置 性别 粉丝 关注 微博 注册时间 认证类型 认证原因}
      @sex = "" #性别
      @tpye_name = "" #认证类型
      @uids = open 'BBB-uid'#文件的路径
      @uids = @uids.read
      @uids = @uids.strip.split("\n")
      @uids.each do |uid|
        begin
          weibo_account = WeiboAccount.where("uid = ?",uid)
          if !weibo_account.blank?
            name = weibo_account[0].screen_name
            location = weibo_account[0].location #位置
            #weibo_account[0].gender == 0 ? @sex = "女" : @sex = "男"
          ...
          #@type_name 有可能 ["微博女郎", nil] 有一个空值
           ...
           @name_str = @name_str[0,@name_str.length-1]
           #verified_reason = weibo_account[0].verified_reason #认证原因(没有支持)
            csv << [uid,name,location,@sex,followers_count,friends_count,statuses_count,created_at,@name_str,nil]
          end
          rescue Exception =>e
          if e.message =~ /User does not exists/ #=~ 用于正则表达式匹配
             csv << [uid,"此用户已被屏蔽"]
...
#============================>>用户基本信息(根据uid)完美收关
  ...
  filename = "intel.csv"
  CSV.open filename,"wb" do |csv|
    uid_url = 'intel-uids-zuixin'
    start_time = '2014-09-31'
    end_time = '2014-10-31'
    ...
    uids.each{|uid|
     ...
     if !(forward_weibo.count == 0)  
        forward_weibo.each do |forward|
          ...
          weibo_id = WeiboMidUtil.str_to_mid URI.parse(record.url.strip).path.split("/").last
          wp = find_by_weibo_id(weibo_id)
          tag1 = wp.tag1
          tag2 = wp.tag2
          tag3 = wp.tag3
          tag4 = wp.tag1
  ...
  CSV.open filename,"wb" do |csv|
    ...
    @uids.each do |name|
      ...
      fans_num = WeiboUserRelation.where("uid = ?",weibo_account.uid).count
      csv << [name,weibo_account.uid,fans_num]
    end
  end
          ...
          begin
            ...
            res['statuses'].each{|w|
              if w.id == top_id
                ...
                url = "http://weibo.com/#{z_uid}/#{WeiboMidUtil.mid_to_str(w.id.to_s)}"
...
#==================================================> 微博互动信息(根据url) 
  ...
  #============================================>>判断一批人是否关注主号
    #3216909083,2788698850,2637370927,2637370247
    filename ='xph全量粉丝判断.csv'
    CSV.open filename,"wb" do |csv|
      csv << %w{uid 之前粉主号时间 距离现在天数 之前是否是粉丝（Y/N） 现在是否是主号粉丝（Y/N） 主号是否关注他（Y/N）}
      z_uid = 2637370247
      task = GetUserTagsTask.new
      ...
      uids.each{|uid|
        ...
        begin
          res = task.stable({retry_interval:1}){
            array = old_csv[rand(0..old_csv.size-1)]
            index = rand(0..old_csv.size-1)
            $GLOBAL_API_IP = array[0] #获得随机ip
            $GLOBAL_API_PORT = array[1] #获得随机端口
            task.api.friendships.show(source_id:uid,target_id:z_uid)
          }
          ...
          rescue Timeout::Error,OpenURI::HTTPError,Errno::ETIMEDOUT,Errno::ECONNRESET,
          Faraday::Error::ConnectionFailed,JSON::ParserError, EOFError,OpenSSL::SSL::SSLError,
          OAuth2::Error, MultiJson::LoadError=>e
            old_csv.delete_at(index) #删除指定代理
            csv << [uid,'','','目标用户不存在','']
        end
      }
    end
    ...
    uids.each{|uid|
      ...
      begin
        ...
        if fans.blank?
         ...
         followedtime = fans.follow_time.to_date
         day = (now_time-time).to_i
         isfans = fans.nil?? "N" :"Y"
         csv << [uid,followedtime,day,isfans,istargetfans, issourcefans]
        end
        rescue Exception=>e
          puts e
          csv << [uid,'','','目标用户不存在','']
 ...
 filename ='是否关注中国.csv'
  ...
  filename ='gw3.csv'
    ...
    uids.each{|uid|
      uid = uid.strip
      fans = WeiboUserRelation.where("uid = ? and by_uid = ?",target_uid,uid)
  ...
  filename ='gw2.csv'
    ...
    begin
      ...
      if fans.blank?
       ...
       followedtime = fans.follow_time.to_s
       isfans = fans.nil?? "N" :"Y"
       cvs << [uid,followedtime,isfans,istargetfans, issourcefans]
      end
      rescue Exception=>e
      puts e
      csv << [uid,'','','目标用户不存在','']
